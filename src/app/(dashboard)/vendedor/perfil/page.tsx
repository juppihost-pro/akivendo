...
const MiniMapa = dynamic(() => import("@/components/mapa/MiniMapa"), {
  ssr: false,
  loading: () => <div style={{ height:180, background:"var(--bg3)", borderRadius:16 }} />,
});
...
export default function VendedorPerfilPage() {
  ...
  const [usuarioL,  setUsuarioL]  = useState<any>(null);
  const [tiendaL,   setTiendaL]   = useState<any>(null);
  const [formT,     setFormT]      = useState({
    nombre:"", descripcion:"", categoria_id:"ambulante",
    horario:"", whatsapp:"", es_ambulante:true,
  });
  ...
  async function guardar() {
    ...
    if (imgFile) {
      ...
      if (!se) {
        const { data: ud } = sb.storage.from("logos-tiendas").getPublicUrl(path);
    ...
    if (tiendaL) {
      ...
      const { data: nueva, error } = await sb.from("tiendas")
        .insert(payload).select("*, categoria:categorias_vendedor(*)").single();
      if (error) { setErrorMsg(error.message); setGuardando(false); return; }
      if (nueva) { setTiendaL(nueva); setTienda(nueva); }
    }

    setEditando(false); setGuardando(false);
    const { data: t } = await sb.from("tiendas")
      .select("*, categoria:categorias_vendedor(*)").eq("vendedor_id", user.id).single();
  ...
  async function toggleActiva() {
    if (!tiendaL) return;
    const sb = createClient();
    const actualizada = { ...tiendaL, activa: !tiendaL.activa };
    await sb.from("tiendas").update({ activa: !tiendaL.activa }).eq("id", tiendaL.id);
    setTiendaL(actualizada);
  ...
  const inp: React.CSSProperties = {
    ...
    fontFamily:"Montserrat,sans-serif", fontSize:14, color:"var(--text)", outline:"none",
  };
  ...
  if (loading) return (
    <PageShell title="Mi tienda">
      <p style={{ textAlign:"center", padding:"60px 0", color:"var(--muted)" }}>Cargando...</p>
    </PageShell>
  );
          ...
          <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:20, padding:20, boxShadow:"var(--shadow-sm)" }}>
            <div style={{ display:"flex", gap:14, alignItems:"flex-start", marginBottom:14 }}>
              <div style={{ width:60, height:60, borderRadius:16, background:"var(--bg3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:28, border:"1px solid var(--border)", overflow:"hidden", flexShrink:0 }}>
                {tiendaL.logo_url
                  ? <img src={tiendaL.logo_url} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }} />
                  : CATEGORIAS_VENDEDOR.find(c => c.id === tiendaL.categoria_id)?.emoji ?? "🏪"}
              </div>
              <div style={{ flex:1 }}>
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={() => setEditando(true)}
                style={{ flex:1, padding:"12px", borderRadius:100, border:"1.5px solid var(--border)", background:"transparent", color:"var(--text)", fontFamily:"Montserrat,sans-serif", fontWeight:700, fontSize:13, cursor:"pointer" }}>
                ✏️ Editar
              </button>
              <button onClick={toggleActiva}
                style={{ flex:1, padding:"12px", borderRadius:100, border:"none",
                  background: tiendaL.activa ? "rgba(239,68,68,0.1)" : "var(--teal)",
                  color: tiendaL.activa ? "#dc2626" : "#fff",
                  fontFamily:"Montserrat,sans-serif", fontWeight:800, fontSize:13, cursor:"pointer",
                  border: tiendaL.activa ? "1.5px solid rgba(239,68,68,0.3)" : "none" }}>
            ...
            <div style={{ display:"flex", gap:10 }}>
              {tiendaL && (
                <button onClick={() => setEditando(false)}
                  style={{ flex:1, padding:"13px", borderRadius:100, border:"1.5px solid var(--border)", background:"transparent", color:"var(--text)", fontFamily:"Montserrat,sans-serif", fontWeight:700, fontSize:13, cursor:"pointer" }}>
                  Cancelar
                </button>
              )}
              <button onClick={guardar} disabled={guardando || !formT.nombre || !formT.whatsapp}
                style={{ flex:2, padding:"13px", borderRadius:100, border:"none",
                  ...
                  boxShadow:"0 4px 14px rgba(0,184,138,0.3)",
                  opacity: (guardando || !formT.nombre || !formT.whatsapp) ? 0.65 : 1 }}>
